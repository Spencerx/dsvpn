...
static void permute_neon(uint32_t st[12])
{
#ifdef __aarch64__
    const uint8x16_t rhoEast2 = { 11, 8, 9, 10, 15, 12, 13, 14, 3, 0, 1, 2, 7, 4, 5, 6 };
#endif
    ...
    for (r = 0; r < XOODOO_ROUNDS; r++) {
        ...
        b = ROL32in128(d, 1);
#ifdef __aarch64__
        c = vreinterpretq_u32_u8(vqtbl1q_u8(vreinterpretq_u8_u32(c), rhoEast2));
#else
        f = vextq_u32(c, c, 2);
        c = ROL32in128(f, 8);
#endif
...
#define permute_short permute_neon
#if !defined(__APPLE__) || !defined(__aarch64__)
#define permute permute_neon
#endif
#endif

#if (defined(__APPLE__) && defined(__aarch64__)) || \
    (!defined(__SSSE3__) && !defined(__ARM_NEON) && !defined(__aarch64__))
...
#ifndef permute_short
#define permute_short permute
#endif
...
#elif defined(__ARM_NEON) || defined(__aarch64__)
    vst1q_u8((uint8_t *) out,
             veorq_u8(vld1q_u8((const uint8_t *) out), vld1q_u8((const uint8_t *) in)));
#else
...
static inline void squeeze_permute(uint32_t st[12], unsigned char dst[16], int short_msg)
{
    endian_swap_rate(st);
    memcpy(dst, st, 16);
    endian_swap_rate(st);
...
static inline void store32le(unsigned char *dst, uint32_t value)
{
#ifdef NATIVE_BIG_ENDIAN
    value = byteswap32(value);
#endif
...
static inline void encrypt_block(uint32_t st[12], unsigned char msg[16])
{
    for (unsigned i = 0; i < 4; i++) {
        st[i] ^= load32le(msg + 4 * i);
        store32le(msg + 4 * i, st[i]);
    }
}

static inline void decrypt_block(uint32_t st[12], unsigned char msg[16])
{
    for (unsigned i = 0; i < 4; i++) {
        uint32_t ciphertext = load32le(msg + 4 * i);
        uint32_t plaintext  = st[i] ^ ciphertext;

        st[i] = ciphertext;
        store32le(msg + 4 * i, plaintext);
    }
}

static inline void absorb_block(uint32_t st[12], const unsigned char msg[16])
{
    for (unsigned i = 0; i < 4; i++) {
        st[i] ^= load32le(msg + 4 * i);
    }
...
void uc_encrypt(uint32_t st_[12], unsigned char *msg, size_t msg_len, unsigned char tag[16])
{
    uint32_t      st[12];
    ...
    permute(st);
    squeeze_permute(st, tag, msg_len <= 16);
    memcpy(st_, st, sizeof st);
}

int uc_decrypt(uint32_t st_[12], unsigned char *msg, size_t msg_len,
...
void uc_hash(uint32_t st_[12], unsigned char h[32], const unsigned char *msg, size_t len)
{
    uint32_t      st[12];
    unsigned char padded[16 + 1];
    size_t        off = 0;
    ...
    permute(st);
    squeeze_permute(st, &h[0], 0);
    squeeze_permute(st, &h[16], len <= 16);
    memcpy(st_, st, sizeof st);
}
...
void uc_memzero(void *buf, size_t len)
{
    ...
    if (len >= 16 && len <= 32) {
        memset(buf, 0, 16);
        memset((unsigned char *) buf + len - 16, 0, 16);
    } else {
        memset(buf, 0, len);
    }
    __asm__ __volatile__("" : : "r"(buf) : "memory");
#else
    volatile unsigned char *buf_ = (volatile unsigned char *) buf;
